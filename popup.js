document.addEventListener('DOMContentLoaded', function () {
  ...
  const toggleMostRelevant = document.getElementById('toggleMostRelevant');
  ...
  toggleClutter.addEventListener('change', function () {
    // Save the enable/disable state of the home page clutter feature
    chrome.storage.sync.set({ 'isEnabledClutter': toggleClutter.checked });
  });

  toggleMostRelevant.addEventListener('change', function () {
    chrome.storage.sync.set({ 'isEnabledMostRelevant': toggleMostRelevant.checked });
  });

  // Fetches the state of the checkboxes when the user opens the extension
  chrome.storage.sync.get(['isEnabled', 'isEnabledStreamVods', 'isEnabledScheduledStreams', 'isEnabledWatchedVideos', 'isEnabledClutter', 'isEnabledMostRelevant'],
    function (data) {
    ...
    toggleClutter.checked = data.isEnabledClutter !== false;
    toggleMostRelevant.checked = data.isEnabledMostRelevant !== false;
  });
});
